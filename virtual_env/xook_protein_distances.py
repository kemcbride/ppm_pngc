...
#INPUT_PATH = '/Vagabundo/monica/notes/virtual_env/directory'
GFF_PATH   = '/research/gmh/GENOME_DB/gff-' + pieces
#LIST_PATH = '/Vagabundo/monica/notes/complete_list'
# COLS = ['target_name', 't_accession', 'tlen', 'query_name', 'q_accession', 'qlen', 'e_full', 'score_full', 'bias_full', 'num_domain', 'of_domain', 'ce_domain', 'ie_domain', 'sc$
...
def new_function(qname, genome_id):
    blastdb = '/research/gmh/GENOME_DB/blastpDB-{}/{}'.format(pieces, genome_id)
...
def main():
    ...
    # Reading all the .out files in gff
    ...
    for fname in out_files:
        try:
            ...
            # We want to only look at the EDK motif in pep rows
            genome_id = fname.split('.')[0]
            pep_mutase_rows = pep_mutase_rows[pep_mutase_rows.apply(lambda row: new_function(row.query_name, genome_id), axis=1)]
